"use strict";var Tt=Object.create;var x=Object.defineProperty;var gt=Object.getOwnPropertyDescriptor;var ft=Object.getOwnPropertyNames;var St=Object.getPrototypeOf,bt=Object.prototype.hasOwnProperty;var ht=(r,e)=>{for(var t in e)x(r,t,{get:e[t],enumerable:!0})},_e=(r,e,t,s)=>{if(e&&typeof e=="object"||typeof e=="function")for(let n of ft(e))!bt.call(r,n)&&n!==t&&x(r,n,{get:()=>e[n],enumerable:!(s=gt(e,n))||s.enumerable});return r};var M=(r,e,t)=>(t=r!=null?Tt(St(r)):{},_e(e||!r||!r.__esModule?x(t,"default",{value:r,enumerable:!0}):t,r)),Ot=r=>_e(x({},"__esModule",{value:!0}),r);var $t={};ht($t,{generateContext:()=>ae});module.exports=Ot($t);var mt=M(require("path"),1),ut=require("os"),lt=require("fs");var Te=require("bun:sqlite");var b=require("path"),K=require("os"),k=require("fs");var N=require("fs"),L=require("path"),pe=require("os"),q=(o=>(o[o.DEBUG=0]="DEBUG",o[o.INFO=1]="INFO",o[o.WARN=2]="WARN",o[o.ERROR=3]="ERROR",o[o.SILENT=4]="SILENT",o))(q||{}),ce=(0,L.join)((0,pe.homedir)(),".claude-mem"),V=class{level=null;useColor;logFilePath=null;logFileInitialized=!1;constructor(){this.useColor=process.stdout.isTTY??!1}ensureLogFileInitialized(){if(!this.logFileInitialized){this.logFileInitialized=!0;try{let e=(0,L.join)(ce,"logs");(0,N.existsSync)(e)||(0,N.mkdirSync)(e,{recursive:!0});let t=new Date().toISOString().split("T")[0];this.logFilePath=(0,L.join)(e,`claude-mem-${t}.log`)}catch(e){console.error("[LOGGER] Failed to initialize log file:",e),this.logFilePath=null}}}getLevel(){if(this.level===null)try{let e=(0,L.join)(ce,"settings.json");if((0,N.existsSync)(e)){let t=(0,N.readFileSync)(e,"utf-8"),n=(JSON.parse(t).CLAUDE_MEM_LOG_LEVEL||"INFO").toUpperCase();this.level=q[n]??1}else this.level=1}catch{this.level=1}return this.level}correlationId(e,t){return`obs-${e}-${t}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.getLevel()===0?`${e.message}
...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let l="";if(n){let{sessionId:T,memorySessionId:O,correlationId:S,...u}=n;Object.keys(u).length>0&&(l=` {${Object.entries(u).map(([E,h])=>`${E}=${h}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${l}${m}`;if(this.logFilePath)try{(0,N.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",l={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},p=new V;function Rt(){let r=process.argv[1];return r?(0,b.dirname)(r):process.cwd()}function Ct(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,b.join)((0,K.homedir)(),".claude-mem"),e=(0,b.join)(r,"settings.json");try{if((0,k.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var C=Ct(),I=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,K.homedir)(),".claude"),Gt=(0,b.join)(I,"plugins","marketplaces","thedotmack"),Ht=(0,b.join)(C,"archives"),Bt=(0,b.join)(C,"logs"),Wt=(0,b.join)(C,"trash"),Yt=(0,b.join)(C,"backups"),qt=(0,b.join)(C,"modes"),Vt=(0,b.join)(C,"settings.json"),me=(0,b.join)(C,"claude-mem.db"),Kt=(0,b.join)(C,"vector-db"),Jt=(0,b.join)(C,"observer-sessions"),Qt=(0,b.join)(I,"settings.json"),zt=(0,b.join)(I,"commands"),Zt=(0,b.join)(I,"CLAUDE.md");function ue(r){(0,k.mkdirSync)(r,{recursive:!0})}function le(){return(0,b.join)(Rt(),"..")}var Ee=require("crypto");var At=3e4;function w(r,e,t){return(0,Ee.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function $(r,e,t){let s=t-At;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var F=class{db;constructor(e=me){e!==":memory:"&&ue(C),this.db=new Te.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,c=s==="date_asc"?"ASC":"DESC",m=n?`LIMIT ${n}`:"",l=e.map(()=>"?").join(","),g=[...e],T=[];if(o&&(T.push("project = ?"),g.push(o)),i)if(Array.isArray(i)){let u=i.map(()=>"?").join(",");T.push(`type IN (${u})`),g.push(...i)}else T.push("type = ?"),g.push(i);if(a){let u=Array.isArray(a)?a:[a],f=u.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...u),T.push(`(${f.join(" OR ")})`)}if(d){let u=Array.isArray(d)?d:[d],f=u.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");u.forEach(E=>{g.push(`%${E}%`,`%${E}%`)}),T.push(`(${f.join(" OR ")})`)}let O=T.length>0?`WHERE id IN (${l}) AND ${T.join(" AND ")}`:`WHERE id IN (${l})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${O}
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let l=[],g=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let u=w(e,S.title,S.narrative),f=$(this.db,u,d);if(f){l.push(f.id);continue}let E=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),o||null,i,u,c,d);l.push(Number(E.lastInsertRowid))}let T=null,O=null;if(n){let S=this.db.prepare(`
          SELECT MAX(created_at_epoch) as max_epoch
          FROM observations WHERE memory_session_id = ?
        `).get(e);O=Math.max(d,(S?.max_epoch??0)+1);let u=new Date(O).toISOString(),E=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let E=w(e,f.title,f.narrative),h=$(this.db,E,m);if(h){T.push(h.id);continue}let de=O.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,E,l,m);T.push(Number(de.lastInsertRowid))}let S;if(n){let f=this.db.prepare(`
          SELECT MAX(created_at_epoch) as max_epoch
          FROM observations WHERE memory_session_id = ?
        `).get(e),E=Math.max(m,(f?.max_epoch??0)+1),h=new Date(E).toISOString(),Et=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,h,E);S=Number(Et.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(m,o),{observationIds:T,summaryId:S,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],m=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&c.push(o),this.db.prepare(`
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let u=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let E=this.db.prepare(u).all(e,...a,s+1),h=this.db.prepare(f).all(e,...a,n+1);if(E.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=E.length>0?E[E.length-1].created_at_epoch:t,c=h.length>0?h[h.length-1].created_at_epoch:t}catch(E){return p.error("DB","Error getting boundary observations",void 0,{error:E,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let u=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let E=this.db.prepare(u).all(t,...a,s),h=this.db.prepare(f).all(t,...a,n+1);if(E.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=E.length>0?E[E.length-1].created_at_epoch:t,c=h.length>0?h[h.length-1].created_at_epoch:t}catch(E){return p.error("DB","Error getting boundary timestamps",void 0,{error:E,project:o}),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
