...
export interface ObservationQueueEngine {
  ...
  getSessionsWithPendingMessages(): Promise<number[]>;
  close(): Promise<void>;
}
...
export interface ObservationQueueInspection {
  ...
  clearPendingComplete(sessionDbId: number): Promise<number>;
}
...
export class SqliteObservationQueueEngine implements InspectableObservationQueueEngine {
  ...
  async getSessionsWithPendingMessages(): Promise<number[]> {
    return this.store.getSessionsWithPendingMessages();
  }
  ...
  async clearPendingComplete(sessionDbId: number): Promise<number> {
    const removed = this.store.clearPendingComplete(sessionDbId);
    if (removed > 0) {
      this.emit(sessionDbId);
    }
    return removed;
