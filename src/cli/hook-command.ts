...
function writeHookOutput(adapter: PlatformAdapter, result: HookResult): void {
...
async function executeHookPipeline(
  ...
  writeHookOutput(adapter, result);
...
export async function hookCommand(platform: string, event: string, options: HookCommandOptions = {}): Promise<number> {
  ...
  try {
    ...
    if (error instanceof AdapterRejectedInput) {
      ...
      writeHookOutput(adapter, { continue: true, suppressOutput: true });
      if (!options.skipExit) {
